...
def get_coordinates_thorough(db_path, description_condition):
    ...
    try:
        with sqlite3.connect(db_path) as conn:
            ...
            """
            cursor.execute(query, (description_condition, description_condition))
...
def get_teacher_data(
    db_path: str, name: str = None, cabin_no: str = None, room_no: str = None
) -> List[Dict[str, Any]]:
...
def check():
    ...
    def update_description(db1_data, db2_path, table_name):
        ...
        db1_dict = {row[1]: row[2] for row in db1_data}
